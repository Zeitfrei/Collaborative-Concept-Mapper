...
(function() {
  ...
  */
  ...
  /*
    Implementation of a Moodle storage handler.
  */

    window.golab.ils.storage.MoodleStorageHandler = (function(_super) {
    __extends(MoodleStorageHandler, _super);

    function MoodleStorageHandler(metadataHandler, urlPrefix) {
      this.urlPrefix = urlPrefix;
      this.createResource = __bind(this.createResource, this);
      MoodleStorageHandler.__super__.constructor.apply(this, arguments);
      if (this.urlPrefix != null) {
        console.log("Initializing MoodleStorageHandler.");
        this;
      } else {
    ...
    MoodleStorageHandler.prototype.readResource = function(cid, gid, cb) {
      var error;
      try {
        return $.ajax({
          type: "POST",
          url: "" + this.urlPrefix + "/lib.php",
          data: {action: 'readResource', cid: cid, gid: gid},
          success: function(resource) {
            console.log("POST success, response:");
            console.log(resource);
            return cb(null, JSON.parse(resource));
          },
    ...
    MoodleStorageHandler.prototype.resourceExists = function(cid, gid, cb) {
      var error;
      try {
        return $.ajax({
          type: "POST",
          data: {action: 'resourceExists', cid: cid, gid: gid},
          url: "" + this.urlPrefix + "/lib.php",
    ...
    MoodleStorageHandler.prototype.createResource = function(content, cb) {
      var error, resource, cid, gid;
      try {
        resource = this.getResourceBundle(content);
        cid = resource.metadata.provider.id;
        gid = resource.metadata.actor.groupId;
        return $.ajax({
          type: "POST",
          url: "" + this.urlPrefix + "/lib.php",
          data: {action: 'createResource', resource: JSON.stringify(resource), cid: cid, gid: gid},
        ...
        error = _error;
        console.log("Something went wrong when writing to Moodle:");
        console.error(error);
        return cb(error);
      }
    };

    MoodleStorageHandler.prototype.logAction = function(activityObject) {
      var error, cid, gid;
      try {
        cid = activityObject.provider.id;
        gid = activityObject.actor.groupId;
        console.log("Logging user action.");
        return $.ajax({
          type: "POST",
          url: "" + this.urlPrefix + "/lib.php",
          data: {action: 'addAction', activity: JSON.stringify(activityObject), cid: cid, gid: gid},
          success: function(responseData, textStatus, jqXHR) {
            // console.log("POST success, response:");
            // console.log(responseData);
          },
